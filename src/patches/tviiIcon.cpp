...
bool find_rpl(OSDynLoad_NotifyData &found_rpl, const std::string &name) {
    ...
    for (const auto &rpl : rpl_info) {
        ...
        if (std::string_view(rpl.name).rfind(name) == (std::string_view(rpl.name).size() - name.size())) {
            found_rpl = rpl;
...
void osdynload_notify_callback(OSDynLoad_Module module, void *ctx,
    ...
    if (reason == OS_DYNLOAD_NOTIFY_LOADED) {
        if (!rpl->name)
            return;
        if (std::string_view(rpl->name).rfind("nn_olv.rpl") != (std::string_view::npos - std::string_view("nn_olv.rpl").length()))
            return;
